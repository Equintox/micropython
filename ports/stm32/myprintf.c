...
static size_t _ntoa_format(char* buffer, char* buf, size_t len, bool negative,
		unsigned int base, size_t maxlen, unsigned int prec, unsigned int width,
		unsigned int flags) {
	if (maxlen == 0U) {
		return 0U;
	}
...
static size_t _ntoa_long(char* buffer, unsigned long value, bool negative,
		unsigned long base, size_t maxlen, unsigned int prec,
	...
	return _ntoa_format(buffer, buf, len, negative, (unsigned int) base, maxlen,
			prec, width, flags);
}

// internal itoa for 'long long' type
#if defined(PRINTF_LONG_LONG_SUPPORT)
static size_t _ntoa_long_long(char* buffer, unsigned long long value,
		bool negative, unsigned long long base, size_t maxlen,
...
static size_t _ftoa(double value, char* buffer, size_t maxlen,
		unsigned int prec, unsigned int width, unsigned int flags) {
	...
	} else if ((diff == (double)0.5) && ((frac == 0) || (frac & 1))) {
...
///////////////////////////////////////////////////////////////////////////////
size_t _vsnprintf(char* buffer, size_t buffer_len, const char* format,
		va_list va) {
	...
	while (idx < buffer_len) {
		...
		case 'b': {
			...
			if ((*format == 'i') || (*format == 'd')) {
				...
				if (flags & FLAGS_LONG_LONG) {
					...
					idx +=
							_ntoa_long_long(&buffer[idx],
									(unsigned long long) (
											value > 0 ? value : 0 - value),
									value < 0, base, buffer_len - idx,
					...
					idx += _ntoa_long(&buffer[idx],
							...
							value < 0, base, buffer_len - idx, precision, width,
							flags);
				} else {
				...
				if (flags & FLAGS_LONG_LONG) {
#if defined(PRINTF_LONG_LONG_SUPPORT)
					idx += _ntoa_long_long(&buffer[idx],
							va_arg(va, unsigned long long), false, base,
							buffer_len - idx, precision, width, flags);
#endif
				} else if (flags & FLAGS_LONG) {
					idx += _ntoa_long(&buffer[idx], va_arg(va, unsigned long),
							false, base, buffer_len - idx, precision, width,
							flags);
				} else {
					idx += _ntoa_long(&buffer[idx], va_arg(va, unsigned int),
							false, base, buffer_len - idx, precision, width,
							flags);
				}
			}
		...
		case 'f':
		case 'F':
			idx += _ftoa(va_arg(va, double), &buffer[idx], buffer_len - idx,
					precision, width, flags);
			format++;
			break;
#endif  // PRINTF_FLOAT_SUPPORT
		...
		case 'p': {
			...
			if (sizeof(uintptr_t) == sizeof(long long)) {
				...
				idx += _ntoa_long(&buffer[idx],
						...
						16U, buffer_len - idx, precision, width, flags);
			}
			format++;
			break;
		}
...
void print(const char ch) {
	mp_obj_t builtin_module_obj = mp_module_get(MP_QSTR_builtins);
	if (builtin_module_obj) {
		mp_obj_t print_fn = mp_load_attr(builtin_module_obj, MP_QSTR_print);
		if (print_fn) {
			mp_call_function_1(print_fn, mp_obj_new_str(&ch, 1));
		}
	}
}
